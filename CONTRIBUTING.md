...
If you're an incredibly awesome person and want to help us make MZinga even better through new features or additions, we would be thrilled to work with you.
...
To help us work on new features, you can create a new feature request post in [GitHub Discussion](https://github.com/mzinga-io/mzinga-core/discussions). New functionality often has large implications across the entire MZinga repo, so it is best to discuss the architecture and approach before starting work on a pull request.
...
Each test directory is split up in this way specifically to reduce friction when creating tests and to add the ability to boot up MZinga with that specific config.

The following command will start MZinga with your config: `pnpm dev my-test-dir`. Example: `pnpm dev fields` for the test/`fields` test suite. This command will start up MZinga using your config and refresh a test database on every restart. If you're using VS Code, the most common run configs are automatically added to your editor - you should be able to find them in your VS Code launch tab.
