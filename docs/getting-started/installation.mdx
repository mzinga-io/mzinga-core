...
label: Installation
order: 30
desc: To quickly get started with Payload, simply run npx create-mzinga-app or install from scratch.
...
## Quickstart with create-mzinga-app

To quickly scaffold a new Payload app in the fastest way possible, you can use [create-mzinga-app](https://www.npmjs.com/package/create-mzinga-app). To do so, run the following command:
...
Looking to deploy Payload with Docker? New projects with `create-mzinga-app` come with a Dockerfile and docker-compose.yml file ready to go. Examples of these files can be seen in our [Deployment docs](/docs/production/deployment#docker).
