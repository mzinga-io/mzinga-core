...
```ts
import { BlocksFeature, LinkFeature, UploadFeature, lexicalEditor } from '@mzinga/richtext-lexical'
...
import { HTMLConverterFeature, lexicalEditor, lexicalHTML } from '@mzinga/richtext-lexical'
...
This method employs `convertLexicalToHTML` from `@mzinga/richtext-lexical`, which converts the serialized editor state into HTML.
...
```ts
import type { HTMLConverter } from '@mzinga/richtext-lexical'
...
The `convertLexicalToHTML` is part of `@mzinga/richtext-lexical` automatically handles traversing the editor state and calling the correct converter for each node.
...
import { getEnabledNodes, sanitizeEditorConfig } from '@mzinga/richtext-lexical'
...
```ts
import { defaultEditorConfig, defaultEditorFeatures } from '@mzinga/richtext-lexical' // <= make sure this package is installed
...
import { sanitizeEditorConfig } from '@mzinga/richtext-lexical'
...
import { SlateToLexicalFeature, lexicalEditor } from '@mzinga/richtext-lexical'
...
import { cloneDeep, convertSlateToLexical, defaultSlateConverters } from '@mzinga/richtext-lexical'
...
import type { SlateNodeConverter } from '@mzinga/richtext-lexical'
