...
```graphql
mutation {
  login[collection-singular-label](email: "contact@mzinga.io", password: "yikes") {
    user {
      email
    }
    exp
...
```
mutation {
  unlock[collection-singular-label](email: "contact@mzinga.io")
}
```
...
```
mutation {
  forgotPassword[collection-singular-label](email: "contact@mzinga.io")
}
```
